...
export class AppController {
    ...
    @Put('add')
    @UseGuards(AuthGuard)
    addToFavourites(@Req() req, @Body() addToFavouritesDto: AddToFavouritesDto): Promise<FavouriteDocument> {
    ...
    @Put('remove')
    @UseGuards(AuthGuard)
    removeFromFavourites(@Req() req, @Body() removeFromFavouritesDto: RemoveFromFavouritesDto): Promise<FavouriteDocument> {
        const {_id: id} = req.user
        return this.favoritesService.removeFromFavourites(id, removeFromFavouritesDto);
    }
